...
public class ProcessManager {
    ...
    public void assignProcessesInitially() {
        for (int i = lastAsigned; i < input_ProcessList.size(); i++) {
    ...
    public ArrayList<Process> execute(ArrayList<Process> auxListProcess) {
        for (int i = 0; i < currentPartitionList.size(); i++) {
            ...
            if (pro == null && (i + 1) < currentPartitionList.size() && currentPartitionList.get(i + 1).getAssignedProcess() == null) {
                condensate(i);
                continue;
            } else if (pro == null && lastAsigned < input_ProcessList.size()) {
                if (input_ProcessList.get(lastAsigned).getProcessSize() < partition.getPartitionSize()) {
                    //Particion 1:
                    Partition newPar1 = new Partition(input_ProcessList.get(lastAsigned));
                    ...
                    Partition newPar2 = new Partition(partition.getPartitionSize() - input_ProcessList.get(lastAsigned).getProcessSize());
                    ...
                    lastAsigned++;
                } else if (input_ProcessList.get(lastAsigned).getProcessSize() == partition.getPartitionSize()) {
                    Partition newPar1 = new Partition(input_ProcessList.get(lastAsigned));
